...
def get_data_file(filetype, day, month, year):
    ...
    for station in stations:
        wgurl = f'https://api.weather.com/v2/pws/history/{filetype}?stationId={station}&format=json&units=m&apiKey={apiKey}&date={day}&numericPrecision=decimal'
        filename = f'data/{station}/{filetype}/{year}/{month}/{day}.json'
        path = f'data/{station}/{filetype}/{year}/{month}/'

        if not os.path.exists(path):
            os.makedirs(path)

        if os.path.isfile(filename):
            print(f'.....already got file for {station}/{filetype}/{day}')
        else:
            print(f'.....downloading file for {station}/{filetype}/{day}')
            time.sleep(5)
